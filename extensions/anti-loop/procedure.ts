...
4. If a response was truncated (max output tokens), continue with one action only — do not restart the whole investigation. Write big files in parts (write, then edit).
5. Do not re-read the same file or re-run the same failing test without changing something first.
6. Never write tool calls as text (<tool_call>, <function=...>, or a {"name": ..., "arguments": ...} JSON block) — text is not executed. Use native tool calls only.
